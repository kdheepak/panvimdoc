...
function test_pandoc(text::String; toc = true, dedup_subheadings = false, treesitter = true, demojify = false)
  ...
  cd(ROOT_DIR) do
    run(
      `$(Pandoc.pandoc()) --metadata=project:"test" --metadata=vimversion:"NVIM v0.8.0" --metadata=toc:$toc --metadata=description:"Test Description" --metadata=dedupsubheadings:$dedup_subheadings --metadata=treesitter:$treesitter  -t $SCRIPTS_DIR/panvimdoc.lua $CUR_DIR/test.md -o test/test.txt`,
    )
  end
